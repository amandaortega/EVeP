...
class RMSE(object):
    def __init__(self, path, sigma_min, sigma_max, delta_min, delta_max, N_min, N_max, rho_min, rho_max):        
        abspath = os.path.abspath(__file__)
        os.chdir(os.path.dirname(abspath))        

        try:
            self.X = genfromtxt(path + 'X_train.csv', delimiter=',')
            self.y = genfromtxt(path + 'Y_train.csv', delimiter=',')
        except Exception:
            self.X = genfromtxt(path + 'X_train.csv', delimiter=' ')
            self.y = genfromtxt(path + 'Y_train.csv', delimiter=' ')
