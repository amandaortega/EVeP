...
class Least_SRMTL2(object):
    ...
    def funVal_eval(self, X, Y, W, R):
        ...
        for i in range(self.t):
            ...
            for j in range(self.t):
                if j != i:
                    v_aux = v_aux + R[i,j] * W[:,j]

            funcReg = funcReg + 0.5 * np.linalg.norm(W[:, i] - v_aux) ** 2

        return funcVal + self.rho_1 * funcReg + self.rho_3 * np.linalg.norm(W, 'fro') ** 2

    def gradVal_eval(self, X, XY, W, B, v_ones_T):
        grad_W = np.zeros((X[0].shape[0], self.t))
        term_reg = np.zeros((X[0].shape[0], self.t))
        ...
        for t_ii in range(self.t):
            ...
            term_reg[:, t_ii] = (W[:,t_ii].reshape(-1, 1) - W @ B[:,:,t_ii] @ v_ones_T).reshape(-1)

            for t_iii in range(self.t):
                if t_iii != t_ii:
                    term_reg[:, t_ii] = term_reg[:, t_ii] + (B[t_ii,t_ii,t_iii] * (-W[:,t_iii].reshape(-1, 1) + W @ B[:,:,t_iii] @ v_ones_T)).reshape(-1)

        return grad_W + self.rho_1 * 2 *  term_reg + self.rho_3 * 2 * W
    ...
    def train(self, X, Y, R, init_theta=2):
        ...
        # precomputation
        B = np.zeros((self.t, self.t, self.t))
        ...
        for i in range(self.t):
            ...
            # normalizing
            if np.sum(v_Bt) > 0:
                v_Bt = v_Bt / np.sum(v_Bt)

            B[:,:,i] = np.diag(v_Bt)

        v_ones_T = np.ones((self.t, 1))
        ...
        for iter_ in range(Least_SRMTL2.DEFAULT_MAX_ITERATION):
            alpha = (t_old - 1) / t
            Ws = (1 + alpha) * Wz - alpha * Wz_old

            # compute the function value and gradients of the search point
            gWs = self.gradVal_eval(X, XY, Ws, B, v_ones_T)
            ...
            # test stop condition
            if iter_ >= 2:
                if abs(self.funcVal[-1] - self.funcVal[-2]) <= Least_SRMTL2.DEFAULT_TOLERANCE * self.funcVal[-2]:
                    break
